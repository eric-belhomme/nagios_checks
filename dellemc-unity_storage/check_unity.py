...
def httpGet(hostaddress, s, check):
    requrl = 'https://'+hostaddress+'/api/types/'+check + \
        '/instances?fields=health,id&per_page=2000&compact=true'
    r = s.get(requrl)
    j = json.loads(r.text)
    ...
    if not j['entries']:
        ...
        for x in j['entries']:
            ...
            s_nagios, s_msg1, s_msg2, s_val, s_exit = NagiosStatus(
                value, descid, desc)
            if (s_nagios != "OK"):
                return (value, descid, desc)
...
def main():
    parser = argparse.ArgumentParser(description="Script for EMC Unity Storage monitoring",
                                     epilog='''Example:  ./check_unity2.py -H 10.0.0.1 -m DAE -u user -p pass''')
    parser.add_argument("-H", "--hostaddress", type=str,
                        required=True, help="Host address for the URL")
    parser.add_argument("-u", "--user", type=str,
                        required=True, help="Username for system login")
    parser.add_argument("-p", "--password", type=str,
                        required=True, help="Password for system login")
    parser.add_argument("-m", "--module", type=str, choices=['battery', 'dae', 'disk', 'dpe', 'ethernetport', 'fan', 'fcport', 'iomodule', 'lcc', 'memorymodule', 'powersupply', 'sasport', 'ssc', 'ssd', 'storageprocessor', 'system', 'uncommittedport'],
                        required=True, help="Requested MODULE for getting status. Possible options are: battery dae disk dpe ethernetport fan fcPort ioModule lcc memoryModule powerSupply sasPort ssc ssd storageProcessor system uncommittedPort")
    ...
    s.headers.update({'Accept': 'application/json',
                      'Content-type': 'application/json', 'X-EMC-REST-CLIENT': 'true'})
    s.verify = False
    ...
    if r.status_code == 200:
        ...
        s.post('https://'+hostaddress+'/api/types/loginSessionInfo/action/logout',
               data=json.dumps({'localCleanupOnly': 'true'}))
        j = json.loads(r.text)
        # return j['logout']
